...
describe "NotificationMailer" do
  ...
  let(:strategy_reminder)   { strategy.strategy_email_reminder }
  let(:take_medication_text) { I18n.t('mailers.notification_mailer.medication_subject', medication_name: medication.name) }
  let(:refill_medication_text) { I18n.t('mailers.notification_mailer.refill_subject', medication_name: medication.name) }
  let(:perform_strategy_text) { I18n.t('mailers.notification_mailer.strategy_subject', strategy_name: strategy.name) }
  let(:weekly_self_care_text) { I18n.t('mailers.notification_mailer.selfcare_subject', strategy_name: strategy.name) }
  ...
  describe "#weekly_self_care" do
    subject(:email) { NotificationMailer.weekly_self_care(strategy_reminder) }

    it { expect(email.to).to eq(["some@user.com"]) }
    it { expect(email.subject).to eq weekly_self_care_text }
  end
