...
RSpec.describe CommentsHelper, type: :helper do
  ...
  describe '#generate_comments' do
    ...
    # This helper builds the expected deleteAction hash to keep the spec clean
    def expected_delete_action(comment_id)
      {
        dataConfirm: 'Are you sure?',
        dataMethod: 'delete',
        link: "/medications/#{comment_id}", # Matches the 'got' output in your logs
        name: 'Delete'
      }
    end
    ...
    context 'Moments' do
      ...
      context 'Comment posted by Moment creator who is logged in' do
        ...
        it 'generates a valid comment object when visibility is all' do
          ...
          expect(helper.generate_comments(Comment.where(id: new_comment.id))).to eq([{
            id: new_comment.id,
            ...
            viewers: nil,
            deleteAction: expected_delete_action(new_comment.id)
          }])
        end
