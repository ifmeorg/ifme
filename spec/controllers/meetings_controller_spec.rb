...
RSpec.describe MeetingsController, type: :controller do
  ...
  describe 'add_to_google_cal' do
    ...
    it 'calls calender_uploader#upload_event' do
      ...
      post(:add_to_google_cal, params: { id: meeting.id })
      ...
      expect(flash[:notice]).to eq(I18n.t('meetings.google_cal.add.success'))
    end
  end

  describe 'remove_from_google_cal' do
    include_context(:logged_in_user)
    let(:user) { create(:user_oauth) }
    let!(:meeting) { create(:meeting) }
    let!(:calendar_uploader) { double }
    ...
    it 'calls calender_uploader#delete_event' do
      expect(CalendarUploader).to receive(:new).with(
        summary: nil,
        date: nil,
        access_token: user.google_access_token,
      ...
      delete(:remove_from_google_cal, params: { id: meeting.id })

      expect(meeting_member.reload.google_cal_event_id).to eq(nil)

      expect(response).to redirect_to(group_path(meeting.group_id))
      expect(flash[:notice]).to eq(I18n.t('meetings.google_cal.remove.success'))
    end
  end
